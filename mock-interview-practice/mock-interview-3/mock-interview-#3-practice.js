...
const swapFirstLastWord = string => {
  ...
  // Swap first and last using array destructuring
...
// WITHOUT REGEX
const removeExtraSpaces2 = string => string.trim().split(' ').filter(word => word).map(word => word.trim()).join(' '); 
console.log(removeExtraSpaces2('Hello'));
console.log(removeExtraSpaces2('        	Hello    World '));
console.log(removeExtraSpaces2('        	JavaScript is           	fun'));
console.log(removeExtraSpaces2(''));
...
const firstDuplicate = array => {
    for(let i = 0; i < array.length; i++) { // start with one element at index i
        for(let j = i + 1; j < array.length; j++) { // then continue onto the next elements after i 
...
reverseStringWords(" ")                         -> ""
*/
const reverseStringWords = string => string.split(' ').map(ele => ele.trim().split('').reverse().join('')).join(' ');
console.log(reverseStringWords('Hello Word'));
...
const findUniques = (arr1, arr2) => {
    ...
    const filtereddArr = concatArr.filter(num => !(arr1.includes(num) && arr2.includes(num))); 
    const set = new Set(filtereddArr); 
...
/*
One-lined Answer ==> [...new Set(arr1.concat(arr2).filter(num => !(arr1.includes(num) && arr2.includes(num))))]' 
*/
...
const countPalindrome = string => {
    if(string.length === 0 || string.length === '') return 0; 
    ...
    return string.split(' ').reduce((accum, ele) => isPalindrome(ele) ? accum + 1 : accum, 0); 
}
...
const countOccurrence = (str1, str2) => {
    let arrOfOccurrences  = []; // create an array to count how many occurrences each letter in the second string AND return the smallest number
    // return the smallest number because it gives us the MINIMUM of times 
    
    for(const char of str2) { // we're looping through str2 b/c --> it's the parameter we need to initially look at before we compare it to str1
       arrOfOccurrences.push(str1.split('').filter(c => c.toLowerCase() === char.toLowerCase()).length); 
       // we've converted our str1 into an array --> used .filter() to filter out the chars that AREN'T within str2  + return the length  
       // After that ==> we'll push the length into our array 
    }
    return arrOfOccurrences.sort()[0];  // return our SORTED array's first (minimum)
}
