...
else if (month === 3 || month === 4 || month === 5) console.log('Spring'); // (month <= 5) --> Better 
else if (month === 6 || month === 7 || month === 8) console.log('Summer'); // (month <= 8)  --> Better
else console.log('Fall');

let season = month === 12 || month === 1 || month === 2
? 'Winter' :
 month <= 5
? 'Spring' :
month <= 8
...
if (brand === 'iPhone') {
    ...
    if (color !== 'White' && color !== 'Black') price += 100;
}
